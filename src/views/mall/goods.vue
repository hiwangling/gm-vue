<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="所属分类" prop="shoptype_name" />
    ...
    <el-dialog :close-on-click-modal="false" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" inline :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="margin-left:50px;">
        ...
        <el-form-item label="商品单位" prop="unit">
          <el-select v-model="dataForm.unit" clearable placeholder="请选择" style="width:180px">
            <el-option v-for="(item,index) in unit" :key="index" :value="String(item.id)" :label="item.name" />
          </el-select>
        </el-form-item>
        <el-form-item label="商品类型" prop="shoptype">
          <el-select v-model="dataForm.shoptype" placeholder="请选择" style="width:180px">
            <el-option v-for="(item,index) in type" :key="index" :label="item.title" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="商品描述" prop="vcnote">
...
<script>
import { typelist, goodslist, goodsadd, goodsedit, goodsdel } from '@/api/goods'
