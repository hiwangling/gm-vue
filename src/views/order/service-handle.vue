<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-button
        v-permission="['GET /api/v1/service_info/service_infos_by_cemetery']"
        class="filter-item"
        type="primary"
        icon="el-icon-search"
        @click="handleFilter"
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="墓主" prop="vcname" width="150">
        <template slot-scope="scope">
          <!--将最后一个字符舍去-->
          {{ scope.row.vcname.substring(0,scope.row.vcname.length-1) }}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="操作" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            v-permission="['GET /api/v1/service_info/get_service_bill_by_cid']"
            type="primary"
            size="mini"
    ...
    >
      ...
      >
        ...
        <el-table-column align="center" label="操作" class-name="small-padding fixed-width" width="130">
          <template slot-scope="scope">
            <el-button v-if="scope.row.resutlstatus == 2 " type="primary" size="mini" @click="handleCat(scope.row.id)">
              查看
            </el-button>
            <el-button
              v-else
              v-permission="['POST /api/v1/servicebill/execute']"
              type="danger"
              size="mini"
