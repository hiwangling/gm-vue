    ...
    getmonth() {
      monthCartogram(this.listQuery)
        .then(res => {
          ...
          this.lineLegendData = {
            expectedData: this.listQuery.contrast_years == null ? [] : [`${this.listQuery.contrast_years}年迁出统计`],
            actualData: [`${this.year}年迁出统计`]
          }
          this.lineChartTitle = `迁出统计`
        })
    },
    gettogram() {
      classifyCartogram(this.listQuery)
        .then(res => {
          // this.PieChartTitle = '各墓区迁出占比'
          var list = []
          var Actual = []
          var legend = []
